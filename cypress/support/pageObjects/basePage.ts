export default class BasePage {
    ...
    verifyTextVisibility(text: string, visible: boolean = true) {
        visible ? cy.contains(text).should('be.visible') : cy.contains(text).should('not.be.visible')
    }
    ...
    verifyModalVisibility(visible : boolean=true, title?: string) {
        visible ? cy.get(this.MODAL_POPUP_MESSAGE).should('be.visible').and('contain.text', title) : cy.get(this.MODAL_POPUP_MESSAGE).should('not.be.visible')
    }
